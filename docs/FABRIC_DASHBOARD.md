...
- **레이아웃(`compose`):** Header → status `Static` → Horizontal(concept Tree + 우측 panel(`panel-note` + content `Static`/공유 `DataTable`)) → results `RichLog`(초기 접힘) → `StatusFooter`. 넓은 표 뷰(harnesses/models/runtimes/budget/router)는 **하나의 재사용 DataTable**을 공유한다(컬럼 자동 폭·스크롤).
...
- **router actions(v3):** Router 패널은 기본 `router plan --estimated-input-tokens 1000 --no-billable` 후보를 DataTable로 보여주고, `panel-note`에 현재 intent(`no-billable`, estimated input, 선택 route)를 노출한다. Router table은 내부 score를 전면 노출하지 않는다. 행 선택은 `preferred-harness`/`preferred-model` intent로 고정되어 `t`/`E` 실행 modal의 기본값이 된다. `p`/`v`는 intent 모달 입력값으로 `router plan`/`router explain`을 다시 읽어 테이블을 갱신한다. `t`는 `router execute --dry-run --prompt-file -`, `E`는 `router execute --prompt-file -`를 호출하며, prompt는 stdin으로만 전달된다. `E`는 BILLABLE로 분류되어 Cancel-우선 확인 모달을 통과해야 실행된다.
